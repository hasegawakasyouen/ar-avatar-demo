...
**Architecture:** VRM Add-on for Blender（`saturday06/VRM-Addon-for-Blender`、Blender公式拡張機能プラットフォームにも掲載）をBlenderにインストールし、そのPython API（`armature.data.vrm_addon_extension`）をheadlessスクリプトから直接操作してHumanoidボーンマッピング・表情マッピングを設定し、`bpy.ops.export_scene.vrm()`でエクスポートする。GUIでのクリック操作は一切行わない（このBlender環境にはディスプレイが無いため）。

**Tech Stack:** Blender 5.1.2（headless。VRM Add-onはこの環境で実際に有効化・動作確認済み。他に4.5.2 LTS / 2.92も同一PCに存在するが、VRM化作業では5.1を使う）、VRM Add-on for Blender v4.3.3、`@pixiv/three-vrm`（検証用ビューアー、CDN経由）。
...
- Blender 5.1には拡張機能インストール用のCLIサブコマンド（`--command extension build` / `--command extension install-file`）が存在し、動作確認済み
- **【重要】VRM Add-onは、GUIの「Get Extensions」タブからインストールしただけでは有効化されない。**「Add-ons」タブで該当項目のチェックボックスを別途オンにする必要があり、さらにその有効化状態はBlenderを一度終了しないとディスク上の設定ファイルに反映されない（実際にこの手順を踏むまで、headlessスクリプトから`vrm_addon_extension`が一切見えなかった）
- このPCには`Blender 2.92`・`Blender 4.5`・`Blender 5.1`の3つがインストールされている。VRM Add-onは`Blender 5.1`にインストール・有効化した。以降のコマンドはすべて`"C:\Program Files\Blender Foundation\Blender 5.1\blender.exe"`を使う（既存のMixamoパイプライン各スクリプトは引き続き`Blender 4.5`を使用しており、混同しないこと）
...
**状態: 完了済み。** ユーザーがBlender 5.1のGUIから手動でインストール・有効化し、以下のコマンドで動作確認済み:

```bash
"C:\Program Files\Blender Foundation\Blender 5.1\blender.exe" --background --python verify_vrm_addon.py
```

出力: `HAS_EXTENSION: True` / `SPEC_VERSION_DEFAULT: 1.0` / `HAS_VRM1_HUMANOID: True`

このタスクはリポジトリへのコミットは無し（Blender環境のセットアップのみ）。**Task 2から着手してよい。**
...
- [x] **Step 1: スクリプトの基本構造とボーンマッピング部分を書く**
...
- [x] **Step 2: ボーンマッピングだけが正しく動くことを確認する**

```bash
"C:\Program Files\Blender Foundation\Blender 5.1\blender.exe" --background --python "C:\Users\PC_User\Documents\ar-avatar-demo\scripts\export_vrm.py" -- "D:\VRアバター\アバター\リリカアバター\解凍\身体\ririka_v1.0.9\ririka\ririka.blend" "C:\Users\PC_User\Documents\ar-avatar-demo\model.vrm" 2>&1 | tail -20
```
...
- [x] **Step 4: 実行してVRMファイルが生成されることを確認する**
...
Expected: 上記Step 3の注記の通り、スパースアクセサ最適化後のサイズ（約109MB）であること。313MB近くまで膨らんでいる場合は`enable_advanced_preferences`/`export_try_sparse_sk`のどちらかが渡されていない（両方セットでないと無効化される）ため、エクスポート呼び出しを確認する

- [x] **Step 5: 出力ファイルが有効なVRM（glTFバイナリ）構造であることを軽く検証する**
...
- [x] **Step 2: Previewツールで表示・表情切替を確認する**
...
**トラブルシューティング:** `preview_console_logs`に`Failed to resolve module specifier`等のモジュール解決エラーが出た場合、esm.shが`three/examples/jsm/loaders/GLTFLoader.js`のサブパスを想定通りに配信していない可能性がある。その場合は`https://cdn.jsdelivr.net/npm/three@0.169.0/build/three.module.js`と`https://cdn.jsdelivr.net/npm/three@0.169.0/examples/jsm/loaders/GLTFLoader.js`（vrc-mascot-pwaのvendor化で実績のあるjsDelivr経由のURL）に差し替えて再試行する。当て推量で別のCDNを次々試すのではなく、まずブラウザのコンソールエラーの内容を確認してから対処すること。
