...
3. **自動リギング画面**
   ...
   > **Shape Keys（ブレンドシェイプ）について:**
   > VRChatアバターに顔アニメーションの Shape Keys がある場合、Decimate を Apply する前に削除してください。Blenderの Decimate Modifier は Shape Keys がある状態では Apply できないため、先に削除が必要です。このデモでは本体の idle/ダンスアニメーションのみを再生し、顔表情は不要なため、削除しても構いません。
...
7. File > Browse → 見つけた PNG テクスチャをロード
...
**このテンプレートは2026年7月時点で iPhone での実機確認済みです。**

元アバターにより、以下のステップで予期しない問題が生じる可能性があります。

1. **Mixamoのリギング失敗** → Step 2 でボーン削除を確認
2. **Mixamoのリギング時間** → Step 3 で Decimate を試す
3. **テクスチャ欠落** → Step 4 でテクスチャ再リンク
4. **ファイルサイズ** → Step 4 でテクスチャリサイズ

いずれの問題も、このREADMEの該当ステップで解決方法が書かれています。
